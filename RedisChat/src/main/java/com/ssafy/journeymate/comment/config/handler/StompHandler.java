package com.ssafy.journeymate.comment.config.handler;
// import ... 생략
...
public class StompHandler implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if (StompCommand.CONNECT == accessor.getCommand()) { // websocket 연결요청
            ...
            String mateId = commentService.getMateId(Optional.ofNullable((String) message.getHeaders().get("simpDestination")).orElse("InvalidMateId"));
            // 채팅방에 들어온 클라이언트 sessionId를 mateId와 맵핑해 놓는다.(나중에 특정 세션이 어떤 채팅방에 들어가 있는지 알기 위함)
            String sessionId = (String) message.getHeaders().get("simpSessionId");
            mateRepository.setUserEnterInfo(sessionId, mateId);
            // 채팅방의 인원수를 +1한다.
            mateRepository.plusUserCount(mateId);
            ...
            String sessionId = (String) message.getHeaders().get("simpSessionId");
            String mateId = mateRepository.getUserEnterMateId(sessionId);
            // 채팅방의 인원수를 -1한다.
